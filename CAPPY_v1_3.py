...
“”“Delete **pycache** folders and .pyc/.pyo files under root. Returns count removed (best-effort).”””
root = Path(root)
removed = 0
try:
# Remove **pycache** directories
for d in root.rglob(”**pycache**”):
try:
...
def **init**(self, cfg: dict, data_dir: Path):
self.cfg = cfg
...
srcJ_name = str(t.get("sourceJ", "TRIG_EXTERNAL"))
slopeJ_name = str(t.get("slopeJ", "TRIGGER_SLOPE_POSITIVE"))
levelJ_val = int(t.get("levelJ", 128))
srcK_name = str(t.get("sourceK", "TRIG_DISABLE"))
slopeK_name = str(t.get("slopeK", "TRIGGER_SLOPE_POSITIVE"))
levelK_val = int(t.get("levelK", 128))

print(f"[CAPPY] Trigger config: operation={t.get('operation','?')} "
      f"sourceJ={srcJ_name} slopeJ={slopeJ_name} levelJ={levelJ_val} "
      f"sourceK={srcK_name} slopeK={slopeK_name} levelK={levelK_val}")

board.setTriggerOperation(
    operation,
    engine1,
    ats_const(srcJ_name),
    ats_const(slopeJ_name),
    levelJ_val,
    engine2,
    ats_const(srcK_name),
    ats_const(slopeK_name),
    levelK_val,
)
...
print(f"[CAPPY] External trigger: coupling={ext_coupling_name} range={ext_range_name}")

board.setExternalTrigger(
    ats_const(ext_coupling_name),
    ats_const(ext_range_name)
)
...
print(f"[CAPPY] Trigger timeout: timeout_ms={timeout_ms} ({'wait forever' if timeout_ms == 0 else 'auto-trigger'})")
...
def **init**(self, data_dir: Path, master=None):
super().**init**(master)
...
“””
def **init**(self, master, data_dir_var: tk.StringVar, on_status=None):
super().**init**(master, padding=6)
...
“””
def **init**(self, master, launcher, **kwargs):
...
```
    ...
    self._add_spinbox(trig, 5, "Timeout pause (s)", self.var_timeout_pause_s, from_=0.0, to=3600.0, increment=0.1, width=10)
    ...
    self._add_spinbox(trig, 8, "Trigger delay (µs)", self.var_trigger_delay_us, from_=0.0, to=100000.0, increment=0.1, width=10)
...
def load_from_cfg(self, cfg: Dict[str, Any]) -> None:
    ...
    try:
        ...
        self.var_trig_level_pct.set(round((float(level_code) / 255.0) * 100.0, 3))
...
def apply_to_cfg(self, cfg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    trig["timeout_ms"] = 0  # External trigger only — wait forever
...
def **init**(self, script_path: Path):
super().**init**()
